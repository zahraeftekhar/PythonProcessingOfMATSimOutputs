...
# startTime = time.time()
# for q in files: #35 min for each file
...
# # # from groundTruthAnalysis_testBayesianForStay import train_activityData,train_tripData
# # # train_activityData.to_csv('D:/ax/gis/completePLUdata_30sec/clusterPLU_30sec/train_activityData_0.01ratio.CSV', index=False, header=True)
# # # train_tripData.to_csv('D:/ax/gis/completePLUdata_30sec/clusterPLU_30sec/train_tripData_0.01ratio.CSV', index=False, header=True)
# startTime = time.time()
# # _________importing required***** USER *****train data ________________
# from GTAnalysis_userSampling_homeWorkDistribution import train_activityData, train_tripData, train_homeData, train_otherData,train_workData
...
#     clusterData['stay'] = np.log(prior_activity)+np.log( gaussian_kde(train_activityData['Duration(hour)'],bw_method=.1).pdf((clusterData['duration(sec)'].astype(int))/3600))+np.log( gaussian_kde(train_activityData['start(hour)'],bw_method=.1).pdf((clusterData['TIME'].astype(int))/3600))
#     clusterData['pass-by'] = np.log(prior_trip)+np.log( gaussian_kde(train_tripData['Duration(hour)'],bw_method=.1).pdf((clusterData['duration(sec)'].astype(int))/3600))+np.log( gaussian_kde(train_tripData['start(hour)'],bw_method=.1).pdf((clusterData['TIME'].astype(int))/3600))
...
#                 gaussian_kde(train_homeData['Duration(hour)'], bw_method=0.5).pdf((clusterData.loc[stayIndex[n],['duration(sec)']].astype(int))/3600)) + np.log(
#                 gaussian_kde(train_homeData['start(hour)'], bw_method=0.5).pdf((clusterData.loc[stayIndex[n],['TIME']].astype(int))/3600))
#             clusterData.loc[stayIndex[n],['work']] = np.log(prior_work) + np.log(
#                 gaussian_kde(train_workData['Duration(hour)'], bw_method=0.5).pdf((clusterData.loc[stayIndex[n],['duration(sec)']].astype(int))/3600)) + np.log(
#                 gaussian_kde(train_workData['start(hour)'], bw_method=0.5).pdf((clusterData.loc[stayIndex[n],['TIME']].astype(int))/3600))
#             clusterData.loc[stayIndex[n],['other']] = np.log(prior_other) + np.log(
#                 gaussian_kde(train_otherData['Duration(hour)'], bw_method=0.5).pdf((clusterData.loc[stayIndex[n],['duration(sec)']].astype(int))/3600)) + np.log(
#                 gaussian_kde(train_otherData['start(hour)'], bw_method=0.5).pdf((clusterData.loc[stayIndex[n],['TIME']].astype(int))/3600))
...
#                 if anchorLocs.loc[j,'EASTING']==homeEASTING:
#                     anchorLocs.loc[j,'activity'] = 'home'
#     test = clusterData[clusterData['VEHICLE']==173]
#     print((int((time.time() - startTime) / 60)), end=' min ____ end for file number {number}\n'.format(number=r))
#     anchorLocs.to_csv('D:/ax/gis/completePLUdata_30sec/clusterPLU_30sec/anchorLocsPLU_30sec_{number}.CSV'.format(number=r), header=True,index=False)
#     clusterData.to_csv('D:/ax/gis/completePLUdata_30sec/clusterPLU_30sec/identifiedClusters/identified_clusterPLU_30sec_{number}.CSV'.format(number=r), header=True,index=False)
